...
def train(learning_rate, use_visdom):
    ...
    # create train loader
    train_dataset_loader = torch.utils.data.DataLoader(dataset=TrainEvalDataset(dataset=train_dataset, transform=trans, image_path=COCO_IMAGE_PATH), batch_size=train_batch_size, shuffle=is_shuffle, num_workers=number_of_workers, collate_fn=coco_collate_fn_for_bidirectional)
